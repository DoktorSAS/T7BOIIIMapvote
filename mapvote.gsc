...
/*
	...
	set mv_gametypes 		"dm;dm.cfg"             // MP ONLY: This dvar can be used to have multiple gametypes with different maps, with this dvar you can load gamemode cfg files
...
function MapvoteSetRotation(map)
{
	if(SessionModeIsZombiesGame())
	{
		setdvar("sv_maprotationcurrent", map.exec_rotation);
		setdvar("sv_maprotation", map.exec_rotation);
	}
	else
	{
		array = strTok(map.gametype, ";");
		str = "";
		if (array.size > 0)
		{
		...
		setdvar("sv_maprotationcurrent", str + " map " + map.mapid);
		setdvar("sv_maprotation", str + " map " + map.mapid);
	}
